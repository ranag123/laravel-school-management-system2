    ...
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            ...
            <div class="flex flex-wrap items-center">
                <a href="{{ route('assessment.marksassign',$id) }}" class="bg-gray-200 text-gray-700 text-sm uppercase py-2 px-4 flex items-center rounded">
        ...
        <div class="w-7/12 mt-8 bg-white rounded border-b-4 border-gray-300">
            ...
            @foreach ($asses  as $key=>$value)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    <div class="w-1/12 px-4 py-2 text-sm font-semibold text-gray-600 tracking-tight">{{$i}}</div>
                    <div class="w-3/12 px-4 py-2 text-sm font-semibold text-gray-600 tracking-tight">{{$value['user']['name']}} </div>
                    <div class="w-2/12 px-4 py-2 text-sm font-semibold text-gray-600 tracking-tight">{{$value['obt_marks']}} </div>
                    <div class="w-2/12 px-4 py-2 text-sm font-semibold text-gray-600 tracking-tight">{{date('Y.m.d',strtotime($value['created_at']))}} </div>
                     <div class="w-4/12 flex items-center justify-end px-3">

                            @role('Admin|Teacher')
                            <a href="{{ route('assessment.updatemarks',$value['id']) }}" class="ml-1">
                            ...
                            <a href="{{ route('assessment.deletemarks', $value['id']) }}" data-url="{!! URL::route('student.destroy', $value['id']) !!}" class="deletestudent ml-1 bg-gray-600 block p-1 border border-gray-600 rounded-sm">
